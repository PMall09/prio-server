...
variable "pure_gcp" {
  type        = bool
  default     = false
  description = "A data share processor that runs on GCP (i.e., use_aws=false) will still manage some resources in AWS (IAM roles) _unless_ this variable is set to true."
}
...
variable "manifest_domain" {
  type        = string
  description = "Domain (plus optional relative path) to which the global and specific manifests of the environment should be uploaded."
}
